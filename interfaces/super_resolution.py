...
class TextSR(base.TextBase):
    def train(self):
        ...
        ########S  val & test on icdar 2015 dataset
        if self.args.icdar2015:
            val_dataset_list, val_loader_list = self.get_val_data_icdar()
        else:
        ...
        ###############S
        # 初始化writer变量
        writer = SummaryWriter(comment='_'+self.args.vis_dir)
        #writer.add_graph(model, input_to_model=train_loader, verbose=False)

        for epoch in tqdm(range(cfg.epochs),desc='Processing',unit="imgs"):

            #########################S
            # learning rate is decayed by a factor of 10 every half of total epochs
            if (epoch+1) % (cfg.epochs / 2) == 0:
            ...
            for j, data in (enumerate(train_loader)):
                ...
                ###########S
                writer.add_scalar('train/epoch_loss', loss_im, epoch)
                writer.add_scalar('train/iter_loss', loss_im, iters)
                ...
                if iters % cfg.VAL.valInterval == 0:
                    ...
                    for k, val_loader in enumerate(val_loader_list):
                        ...
                        writer.add_scalar('val/psnr', metrics_dict['psnr_avg'], epoch)
                        writer.add_scalar('val/ssim', metrics_dict['ssim_avg'], epoch)
                    ...
                    ####S
                    writer.add_scalar('val/acc', sum(current_acc_dict.values())/3, epoch)
    ...
    def test(self):
        ...
        ########S  val & test on icdar 2015 dataset
        if self.args.icdar2015:
            test_data, test_loader = self.get_test_data_icdar(self.test_data_dir)
        else:
        ...
        for i, data in (enumerate(test_loader)):
            ...
            ###################
            print('libtorch begin')
            traced_script_module = torch.jit.trace(model,images_lr)
            traced_script_module.save("modeltsrres.pt")
            print('libtorch end')
            return -1
            ################################
            ...
            self.make_pic_display(images_lr, images_sr, images_hr, pred_str_lr, pred_str_sr, label_strs, str_filt)
        time_end = time.time()
        psnr_avg = sum(metric_dict['psnr']) / len(metric_dict['psnr'])
        ssim_avg = sum(metric_dict['ssim']) / len(metric_dict['ssim'])
