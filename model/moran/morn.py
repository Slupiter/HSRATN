    ...
    def forward(self, x, test, enhance=1, debug=False):
        ...
        if debug:
            ...
            for i in range(x.size(0)):
                ...
                img_small = x_small[i].data.to(self.device).mul_(0.5).add_(0.5)
                ...
                ############S .cpu()  =====>.to(self.device)
                img_offsets = (offsets_grid[i]).view(1, self.targetH, self.targetW).data.to(self.device).add_(-v_min).mul_(1./(v_max-v_min))
                ...
                img_processed = x_rectified[i].data.to(self.device).mul_(0.5).add_(0.5)
