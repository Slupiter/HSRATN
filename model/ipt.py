...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, patch_size=4, in_channels=64):
        super().__init__()
...
class ImageProcessingTransformer(nn.Module):
    ...
    """
    def __init__(self,mask=False,STN=False,h=16,w=64, patch_size=4, in_channels=3, mid_channels=64, num_classes=1000, depth=12,
        ...
        self.embed_dim = patch_size ** 2 * mid_channels
        if self.embed_dim % num_heads != 0:
            raise RuntimeError("Embedding dim must be devided by numbers of heads")

        num_patches = (h // patch_size)*(w // patch_size)
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches , self.embed_dim))
        self.task_embed = nn.Parameter(torch.zeros(6, 1, num_patches, self.embed_dim))
        ...
        self.de_patch_embedding = nn.Sequential(
            ...
            Rearrange('b (h w) (p1 p2 c) -> b c (h p1) (w p2)', h=h//patch_size,w=w// patch_size, p1=patch_size, p2=patch_size),
        )
    ...
    def forward(self, x):
        ...
        # x, ori_shape = self.to_patch_embedding(x)
