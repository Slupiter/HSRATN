...
class HRAB(nn.Module):
    def __init__(self, conv=common_hran.default_conv, n_feats=64):
        ...
        self.gru1 = GruBlock(n_feats, n_feats)
        self.gru2 = GruBlock(n_feats, n_feats)
    def forward(self, x):
        res_x = x
        ...
        output = self.gru1(output.transpose(-1, -2)).transpose(-1, -2)

        return self.gru2(res_x + output)
